...
export const api = {
  ...
  // SQLite：自选 / 设置 / 日志
  watchlist: (groupId = null) => get('/watchlist' + (groupId != null ? `?group_id=${groupId}` : '')),
  watchlistGroups: () => get('/watchlist/groups'),
  watchlistGroupCreate: (name) => send('POST', '/watchlist/groups', { name }),
  watchlistGroupUpdate: (id, data) => send('PUT', `/watchlist/groups/${id}`, data),
  watchlistGroupDelete: (id) => send('DELETE', `/watchlist/groups/${id}`),
  watchlistGroupReorder: (groupIds) => send('POST', '/watchlist/groups/reorder', { group_ids: groupIds }),
  watchlistStockGroups: (code) => get(`/watchlist/stock-groups/${code}`),
  watchlistSetStockGroups: (code, groupIds) => send('POST', '/watchlist/stock-groups', { code, group_ids: groupIds }),
  watchlistInitPresets: () => send('POST', '/watchlist/init-presets'),
  watchlistAdd: (code, groupId = null) => send('POST', '/watchlist', { code, group_id: groupId }),
  watchlistRemove: (code, groupId = null) => send('DELETE', `/watchlist/${code}` + (groupId != null ? `?group_id=${groupId}` : '')),
  watchlistImport: (codes, groupId = null) => send('POST', '/watchlist/import', { codes, group_id: groupId }),
  watchlistClear: (groupId = null) => send('POST', `/watchlist/clear${groupId != null ? `?group_id=${groupId}` : ''}`),

  ...
  <div class="card">
    <div style="display:flex; align-items: baseline; gap: 20px; flex-wrap: wrap;">
      ...
      <span style="color: var(--text-dim); font-size: 13px; display:inline-flex; align-items:center; gap:6px; flex-wrap:wrap">
        今开 <b :class="vsPreClass(detail.open)">{{ fmtPrice(detail.open) }}</b> · 昨收 <b>{{ fmtPrice(detail.prev_close) }}</b>
        <span
    ...
    <div class="kv-grid mt12">
      <div class="kv"><span class="k">最高</span><span class="v" :class="vsPreClass(detail.high)">{{ fmtPrice(detail.high) }}</span></div>
      <div class="kv"><span class="k">最低</span><span class="v" :class="vsPreClass(detail.low)">{{ fmtPrice(detail.low) }}</span></div>
      ...
      <div class="kv"><span class="k" data-tip="成交量 = 当天成交的手数（1手 = 100股）。和成交额配合看：放量上涨健康，放量下跌危险。">成交量</span><span class="v">{{ fmtAmount(detail.volume) }}手</span></div>
      ...
      <div class="kv"><span class="k" data-tip="均价 = 累计成交额 ÷ 累计成交量（当日平均成本）。现价高于均价说明今天买入的人整体浮盈、盘面强势；低于均价说明多数人套着，偏弱。">均价</span><span class="v" :class="vsPreClass(detail.avg_price)">{{ fmtPrice(detail.avg_price) }}</span></div>
...
function vsPreClass(val) {
  if (val == null || props.detail.prev_close == null) return ''
  if (val > props.detail.prev_close) return 'up'
  if (val < props.detail.prev_close) return 'down'
  return ''
}
</script>
